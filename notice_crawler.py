...
def crawl_samsung(page=1, rows_per_page=10):
    """
    삼성증권 공지사항
    POST /ux/kor/customer/notice/notice/getNoticeList.do
    """
    ...
    api_url = (
        ...
        "ux/kor/customer/notice/notice/getNoticeList.do"
    )
    ...
    for key, crawler in crawlers:
        try:
            ...
            if rows:
                ...
                company_file_name = key
                saved_df, new_only_df = save_csv_dedup(rows, company_file_name)
                print(f"회사별 저장 완료: data/{company_file_name}.csv / 총 {len(saved_df)}건 / 신규 {len(new_only_df)}건")
