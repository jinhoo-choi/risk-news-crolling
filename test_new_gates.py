...
# 아래 케이스들은 운영 CSV(exposure_data.csv)의 '여신 유무'라는 구조적 전제에
# 기대고 있다. 잔고 수치를 직접 비교하진 않지만, 장 대리가 CSV를 갱신해
# 예컨대 다원시스에 여신이 생기면 코드가 멀쩡해도 테스트가 깨진다.
# (2026-08-18 갱신 때 채널값 테스트가 실제로 이렇게 깨졌다.)
# 전제가 무너지면 원인이 드러나도록 먼저 확인한다 — 조용히 실패하면
# 데이터 문제를 코드 결함으로 오인해 엉뚱한 곳을 파게 된다.
def _has_yeosin(rows):
    return any((r.get("종목유형") or "") == "여신" for r in rows or [])

_FIXTURE_PREMISE = [("롯데카드", False), ("SK하이닉스", True),
                    ("다원시스", False), ("제이알글로벌리츠", False), ("본느", False)]
for _nm_, _want in _FIXTURE_PREMISE:
    _rows = nm.find_exposure(_nm_, EXPO)
    if not _rows:
        print(f"    ⚠ [픽스처 전제] {_nm_}: 익스포저에서 사라짐 — "
              f"CSV 갱신 영향. 아래 실패는 코드가 아니라 데이터 변동일 수 있음")
    elif _has_yeosin(_rows) != _want:
        print(f"    ⚠ [픽스처 전제 붕괴] {_nm_}: 여신 "
              f"{'있음' if not _want else '없음'}으로 바뀜(기대 "
              f"{'있음' if _want else '없음'}) — CSV 갱신 영향. "
              f"아래 실패는 코드 결함이 아닐 수 있음")
...
# ══ 3-I. 익스포저 채널 분리 전달 (2026-08-13) ════════════════════════
print("\n[3-I] AI 전달 익스포저 (뱅키스·영업점 각자 표기)")
...
# (2026-08-19) 기존엔 운영 CSV(load_exposure_data)의 아이에스동서 실제 잔고를
# 하드코딩해 비교했다. 장 대리가 exposure_data.csv를 갱신하면 잔고가 바뀌어
# 코드가 멀쩡해도 테스트가 깨졌다 — 8/18 갱신에서 실제로 터졌다.
# 검증 대상은 '채널 분리 표기가 창작으로 오인되지 않는가'이지 특정 종목의
# 잔고가 아니므로, 운영 데이터에서 떼어내 고정 픽스처를 쓴다.
_isd = [
    {"종목명": "테스트여신종목", "종목코드": "000001", "종목유형": "여신",
     "뱅잔고": "1", "뱅고객수": "10", "영잔고": "1", "영고객수": "5",
     "잔고(억)": "2", "고객수": "15"},
    {"종목명": "테스트여신종목", "종목코드": "000001", "종목유형": "주식",
     "뱅잔고": "59", "뱅고객수": "1704", "영잔고": "33", "영고객수": "374",
     "잔고(억)": "92", "고객수": "2078"},
]
_t, _b = nm.sanitize_action_numbers(
    "여신 보유 고객(뱅키스 1억원/10명 · 영업점 1억원/5명) 담보비율 점검", _isd, "")
chk("채널", "채널값 수치검증 통과", not _t, str(_b))
_t2, _b2 = nm.sanitize_action_numbers(
    "주식 보유 고객(뱅키스 59억원/1,704명) 평가손 산출", _isd, "")
chk("채널", "주식 채널값 통과", not _t2, str(_b2))
