...
print("\n[J] 고객문구 차단 — 종목 리스크가 아닌 기사")
for name, title, ent, expect in [
    ("★당사 전산오류(8/21)", "한국투자증권, 민원 한 분기 만에 2배…'전산 오류'가 뇌관",
     "한국투자증권", "당사 기사"),
    ("★카카오페이증권 오안내(8/26)", '"7600만원 넣으면 된다면서요"…15억 날린 투자자, 반대매매 오안내',
     "카카오페이증권", "타 증권사 기사"),
    ("한국금융지주", "한국금융지주 실적 부진", "한국금융지주", "당사 기사"),
    ("정상 종목 — 코람코", "코람코, 더원리츠 투자자에 이례적 경고", "코람코더원리츠", ""),
    ("정상 종목 — 듀오백", "거래소 듀오백, 20일부터 주권매매거래 정지", "듀오백", ""),
    # 경쟁사가 가해자이고 피해종목이 따로 있는 기사까지 막으면 미탐이 된다
    ("경쟁사 가해·피해종목 별개", "NH투자증권 직원이 DI동일 주가조작 가담", "DI동일", ""),
]:
    got = M.notice_forbidden_reason({"title": title, "entity": ent, "entities": [ent]})
    check(f"{name} → {expect or '허용'}", got == expect, repr(got))


print("\n[K] 매매 권유 제거")
_real = ("[한국투자증권] 긴급 안내 코람코더원리츠는 28일이 배당락일로, 이후 주가가 크게 "
         "하락할 수 있습니다. 특별배당을 받으려면 27일 장 마감 전 매수하셔야 합니다. "
         "보유 수량 및 투자 위험을 즉시 확인하시기 바랍니다.")
_out, _n = M.strip_solicitation(_real)
check("★코람코 매수 권유 문장 제거(8/27 실사례)", _n == 1 and "매수하셔야" not in _out)
check("리스크 경고 문장은 보존", "배당락일" in _out and "하락할 수 있습니다" in _out)
check("행동 유도 보존", "확인하시기 바랍니다" in _out)

_std = ("[한국투자증권] 긴급 안내 보유 종목이 20일부터 매매거래가 정지될 예정입니다. "
        "보유 수량 및 주문 가능 상태를 즉시 확인하시기 바랍니다. 관련 공시는 "
        "KIND(kind.krx.co.kr)에서 확인하시기 바랍니다.")
check("표준 문구는 손대지 않음", M.strip_solicitation(_std) == (_std, 0))
_sell, _sn = M.strip_solicitation("손실이 우려되오니 즉시 매도하셔야 합니다. 보유 수량을 확인하시기 바랍니다.")
